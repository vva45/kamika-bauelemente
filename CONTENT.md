...
| `[~]`  | `public/images/categories/{slug}-hero.jpg`        | One photo per category (8 files). **`entrance-doors` and `windows` are done** — the owner's own photographs. The other six are brand plates — the window-frame motif in the logo colours with the range name — drawn by `node scripts/build-category-heroes.mjs`, which now refuses to overwrite a range that has a real photo (`OWNER_PHOTOS`). They are deliberately graphics, not fake photography, and can stay published until real photos arrive. **3:2** (1500×1000 for the plates): the frame shows them whole, so anything narrower or wider gets cropped — and these photos carry the Kamika wordmark in the top-left corner, which is the first thing a crop eats. | Owner |
