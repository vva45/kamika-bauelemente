...
| ------ | -------------------------------------------- | -------------------------------------------------------------------------- | ----- |
| `[x]`  | `public/pdf/catalogues/*.pdf`                | **Fifteen real catalogues are in place**, covering every range except pergolas: entrance doors (6), shutters/blinds (3), windows (Salamander, IGLO ×2, WIKĘD PCV/ALU), gates (2). Titles, years, page counts and file sizes in `src/data/catalogues.ts` were read from the PDFs, not estimated. | —     |
| `[ ]`  | `public/pdf/catalogues/` — pergolas          | The one missing catalogue. The owner is chasing it; the category page says "in preparation" honestly until it lands. | Owner |
...
following the owner's rule — around 60–70% of the original, NOT half, so quality visibly holds
(photos to max 1800 px / JPEG q80, and an image is only replaced when that actually saves ≥20%).
Rollladen went 27.2→18.9 MB and Fassadenjalousien 20.7→15.1 MB; ROKA Signature, Garagentore and
the panel catalogue turned out to be already efficiently compressed — re-encoding them either
gained nothing or GREW the file, so they were left alone (Signature only dropped 27.4→25.0 MB
from garbage collection). Pre-compression versions of everything touched sit in
`source-catalogues/*-original.pdf`.

**Model → page mapping** is done for every extracted range: entrance doors, shutters, gates,
accessories and the WIKĘD showcase all open the catalogue at the exact page.
...
data sheet went with them, and the Altensteig project no longer lists an installed model — it
will again when the supplier confirms the real one. Every range now runs entirely on the
self-hosted catalogues. The four invented gate products had gone the same way earlier, when the
two Tore catalogues arrived: Garagentore (10 extracted models) and Grundstückszäune (20 named
fence/gate models), generated by `scripts/extract_gates_models.py`.
...
first manufacturer (`src/data/manufacturers.ts`), showing three systems — and all three are
backed by self-hosted documents now: **Ideal 5000 and Ideal 8000** by the supplier's one-page
data sheets in `public/pdf/windows/` (figures re-checked against them 2026-08: they match line
for line), and **Ideal Neo MD** by the IGLO catalogue it is printed in. Nothing on the Aluplast
pages is quoted from memory anymore.
...
Still open on the Aluplast side:

- **A full Aluplast catalogue** would allow a models showcase like WIKĘD's; the one-page sheets
  cover the specs but not the versions per system.
- **The Aluplast logo.** The card carries the neutral typographic plate (like every manufacturer
  card now), NOT the registered logo — reproducing a third party's trademark needs their
  permission. If the owner gets the official logo pack from his Aluplast rep (dealers usually
  may use it), replace `public/images/manufacturers/aluplast.jpg` — same path, no code change.
