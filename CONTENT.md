...
| `[x]`  | `public/images/colours/render.jpg`                | **Done (2026-08)** — a white tilt-and-turn window generated from a prompt written here, approved and uploaded by the owner ("la ventana famosa"; original in `source-photos/`). With a REAL photo the multiply veil showed its flaw: it tinted the garden behind the glass and the wall too, so the tint layer now carries an **alpha mask** (`render-mask.png`, generated by measuring the frame's pixel geometry) and the colour lands on the frame only. If the photo is ever replaced, the mask must be re-measured — and note the CSS default is the ALPHA channel, not luminance: a grey-on-opaque PNG masks nothing. | —     |
